...
document.addEventListener("DOMContentLoaded", function () {
    ...
    let sidebarColorCookie = getCookie('sidebar_color');
    if (sidebarColorCookie) {
        let btn = document.querySelector(`.badge[data-color="${sidebarColorCookie}"]`);
        if (btn) sidebarColor(btn);
    }

    // Restore Sidebar Type
    let sidebarTypeCookie = getCookie('sidebar_type');
    if (sidebarTypeCookie) {
        let btn = document.querySelector(`[data-class="${sidebarTypeCookie}"]`);
        if (btn) sidebarType(btn);
    }

    // Restore Navbar Fixed
    let navbarFixedCookie = getCookie('navbar_fixed');
    if (navbarFixedCookie === 'true') {
        let el = document.getElementById('navbarFixed');
        if (el) {
    ...
    if (darkModeCookie === 'true') {
        let el = document.getElementById('dark-version');
        if (el) {
            el.setAttribute("checked", "true");
            darkMode(el);
        }
...
function navbarFixed(el) {
    ...
    el.dataset.active = newState ? "true" : "false";
    setCookie('navbar_fixed', newState ? 'true' : 'false');
...
window.onload = function() {
  ...
  for (var i = 0; i < ripples.length; i++) {
    ripples[i].addEventListener('click', function(e) {
      ...
      rippleDiv.classList.add('ripple');
      setTimeout(function() {
          if (rippleDiv && rippleDiv.parentElement) {
...
function darkMode(el) {
    ...
    let newState = !isActive;
    el.dataset.active = newState ? "true" : "false";
    setCookie('dark_mode', newState ? 'true' : 'false');
    ...
    if (newState) {
        ...
        for (var i = 0; i < text_btn.length; i++) {
            text_btn[i].classList.replace('text-dark', 'text-white');
        }

        for (var i = 0; i < text_span.length; i++) {
            text_span[i].classList.replace('text-dark', 'text-white');
        }

        for (var i = 0; i < text_strong.length; i++) {
            text_strong[i].classList.replace('text-dark', 'text-white');
        }

        for (var i = 0; i < text_nav_link.length; i++) {
            text_nav_link[i].classList.replace('text-dark', 'text-white');
        }
        ...
        for (var i = 0; i < bg_gray_100.length; i++) {
            bg_gray_100[i].classList.replace('bg-gray-100', 'bg-gray-600');
        }

        for (var i = 0; i < btn_text_dark.length; i++) {
            btn_text_dark[i].classList.replace('text-dark', 'text-white');
        }
        ...
        for (var i = 0; i < hr.length; i++) {
            hr[i].classList.replace('light', 'dark');
        }

        for (var i = 0; i < hr_card.length; i++) {
            hr_card[i].classList.replace('light', 'dark');
        }

        for (var i = 0; i < text_btn.length; i++) {
            text_btn[i].classList.replace('text-white', 'text-dark');
        }

        for (var i = 0; i < text_span_white.length; i++) {
            if (!text_span_white[i].closest('.sidenav') && !text_span_white[i].closest('.card.bg-gradient-dark')) {
                text_span_white[i].classList.replace('text-white', 'text-dark');
            }
        }

        for (var i = 0; i < text_strong_white.length; i++) {
            text_strong_white[i].classList.replace('text-white', 'text-dark');
        }

        for (var i = 0; i < text_nav_link_white.length; i++) {
            if (!text_nav_link_white[i].closest('.sidenav')) {
                text_nav_link_white[i].classList.replace('text-white', 'text-dark');
            }
        }

        for (var i = 0; i < secondary.length; i++) {
            secondary[i].classList.remove('text-white', 'opacity-8');
            secondary[i].classList.add('text-dark');
        }

        for (var i = 0; i < bg_gray_600.length; i++) {
            bg_gray_600[i].classList.replace('bg-gray-600', 'bg-gray-100');
        }
        ...
        for (var i = 0; i < btn_text_white.length; i++) {
            if (!btn_text_white[i].closest('.card.bg-gradient-dark')) {
                btn_text_white[i].classList.replace('text-white', 'text-dark');
            }
        }
